 ...
 * 3. Apple Wi-Fi Accessory Configuration (WAC. Available only on MFi variant of
 * the SDK)
 *
...
static void app_wifi_print_qr(const char *name, const char *pop,
...
static esp_err_t get_device_pop(char *pop, size_t max) {
  ...
  if (err == ESP_OK) {
    snprintf(pop, max, "%02x%02x%02x%02x", eth_mac[2], eth_mac[3], eth_mac[4],
             eth_mac[5]);
    return ESP_OK;
  } else {
    return err;
...
#else
static void app_wac_softap_start(char *ssid) { hap_wifi_softap_start(ssid); }
#endif /* ! CONFIG_APP_WIFI_PROV_TRANSPORT_SOFTAP */
static void app_wac_softap_stop(void) { hap_wifi_softap_stop(); }
...
static void event_handler(void *arg, esp_event_base_t event_base,
  ...
  if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
    ...
    ESP_LOGI(TAG, "Connected with IPv6 Address:" IPV6STR,
...
esp_err_t app_wifi_start(TickType_t ticks_to_wait) {
  ...
  if (!provisioned) {
    ...
    if (err != ESP_OK) {
      ESP_LOGE(
          TAG,
          "Error: %d. Failed to get PoP from NVS, Please perform Claiming.",
          err);
      return err;
    }
    ...
    uint8_t custom_service_uuid[] = {
        /* This is a random uuid. This can be modified if you want to change the
           BLE uuid. */
...
#endif /* CONFIG_APP_WIFI_PROV_TRANSPORT_BLE */
    ESP_LOGI(TAG, "Provisioning Started. Name : %s, POP : %s", service_name,
             pop);
#endif /* USE_UNIFIED_PROVISIONING */
#ifdef USE_WAC_PROVISIONING
    esp_event_handler_register(HAP_WAC_EVENT, ESP_EVENT_ANY_ID, &event_handler,
                               NULL);
    hap_wac_start();
#endif /* USE_WAC_PROVISIONING */
  } else {
